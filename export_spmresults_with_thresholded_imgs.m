...
for i = 1:nstats
    ...
    for iconst = 1:ncontrast
        threshold_info.k=spatial_extent
        fname = [user_input, '_', contrast_fname, '_', contrast_types{iconst},'_', 'p_', cm, pvalstr, '_', num2str(spatial_extent), 'vox'];
        [TabData xSPM] = get_xspm_hc(spm_file, contrast_index(iconst),threshold_info, mask_file, NumMax, DisMax);
        Vo = write_thresholded_img(xSPM.Z, xSPM.XYZ, xSPM.DIM, xSPM.M,fname, fullfile([fname, '.nii']));
        PrintData(TabData, fullfile([fname, '.txt']));
        threshold_info.k = xSPM.uc(3)

        [TabData xSPM] = get_xspm_hc(spm_file, contrast_index(iconst),threshold_info, mask_file, NumMax, DisMax);
        fname = [user_input, '_', contrast_fname, '_', contrast_types{iconst},'_', 'p_', cm, pvalstr, '_', num2str(threshold_info.k), 'vox'];
